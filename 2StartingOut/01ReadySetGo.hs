...
{-We can't do 5 + "llama" because they are not of the same type
but we can do 5 + 4.0 then the integer will adapt to a floating-point number-}
{-All of the previous examples we have been using functions,  For instance, *
...
successor = succ 8
{-The min and max function take two parameters and return for min the one that's lesser and
...
--we need to use parentheses for getting the successor or otherwise we would get only the successor from 9
